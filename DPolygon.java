...
public class DPolygon {
	...
	{		
		...
		for(int i=0; i<x.length; i++)
		{
			...
			newX[i] = (Engine3D.ScreenSize.getWidth()/2 - Calculator.CalcFocusPos[0]) + CalcPos[0] * Screen.zoom;
			newY[i] = (Engine3D.ScreenSize.getHeight()/2 - Calculator.CalcFocusPos[1]) + CalcPos[1] * Screen.zoom;			
			if(Calculator.t < 0)
				draw = false;
		}
