...
public class Screen extends JPanel implements KeyListener, MouseListener, MouseMotionListener, MouseWheelListener{
    ...
    static double[] ViewFrom = new double[] { 28, 28, 3},   
    ...
    static double zoom = 1000, MinZoom = 500, MaxZoom = 2500, MouseX = 0, MouseY = 0, MovementSpeed = 0.2;

    //FPS is primitive, you can set the MaxFPS as high as u want
    ...
    double VertLook = -0.4, HorLook = 3.44, aimSight = 10, HorRotSpeed = 900, VertRotSpeed = 2200, SunPos = 0;
    ...
    {       
        ...
        invisibleMouse();   
        new GenerateTerrain();
        Cubes.add(new Cube(18, -5, 0, 2, 2, 2, Color.red,false));
        //Cubes.add(new Cube(0, 0, 0, 0.02, 0.02, 0.02, Color.red,false));

        //createGrid(true,true);
        //createFrameAxis();
    }   

    public void createGrid(boolean setX,boolean setY){
        int x = 0;
        int y = 0;

        if(setX){
            while(x<10){
                Cubes.add(new Cube(x, 0, 0, 0.02, 9, 0.02, Color.red,false));
                x=x+1;
            }
        }
        ...
        if(setY){
            while(y<10){
                Cubes.add(new Cube(0, y, 0, 9, 0.02, 0.02, Color.green,false));
                y=y+1;
            }
        }
    }

    public void createFrameAxis(){
        Cubes.add(new Cube(0, 0, 0, 5, 0.02, 0.02, Color.red,false));
        Cubes.add(new Cube(0, 0, 0, 0.02, 5, 0.02, Color.green,false));
        Cubes.add(new Cube(0, 0, 0, 0.02, 0.02, 5, Color.blue,false));
    }
    ...
    public void paintComponent(Graphics g)
    {
        ...
        g.fillRect(0, 0, (int)Engine3D.ScreenSize.getWidth(), (int)Engine3D.ScreenSize.getHeight());
        ...
        g.drawString("X-Pos: " + ViewTo[0], 40, 80);
        g.drawString("Y-Pos: " + ViewTo[1], 40, 100);
        g.drawString("Z-Pos: " + ViewTo[2], 40, 120);

        //Rotation

        g.drawString("Y-Rot: " + VertLook, 40, 160);
        g.drawString("Z-Rot: " + HorLook, 40, 180);
    ...
    void drawMouseAim(Graphics g)
    {
        ...
        g.drawLine((int)(Engine3D.ScreenSize.getWidth()/2 - aimSight), (int)(Engine3D.ScreenSize.getHeight()/2), (int)(Engine3D.ScreenSize.getWidth()/2 + aimSight), (int)(Engine3D.ScreenSize.getHeight()/2));
        g.drawLine((int)(Engine3D.ScreenSize.getWidth()/2), (int)(Engine3D.ScreenSize.getHeight()/2 - aimSight), (int)(Engine3D.ScreenSize.getWidth()/2), (int)(Engine3D.ScreenSize.getHeight()/2 + aimSight));         
    }
    ...
    {       
        double difX = (NewMouseX - Engine3D.ScreenSize.getWidth()/2);
        double difY = (NewMouseY - Engine3D.ScreenSize.getHeight()/2);
    ...
    void CenterMouse() 
    {
        try {
            r = new Robot();
            r.mouseMove((int)Engine3D.ScreenSize.getWidth()/2, (int)Engine3D.ScreenSize.getHeight()/2);
